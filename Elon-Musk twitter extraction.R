...
cred <- OAuthFactory$new(consumerKey='swFC3PYt7hh5F6EGtPg7ATnQx', # Consumer Key (API Key)
                         consumerSecret='682qgTTOArAShjpTqfkjFvH27Zj4NEvhz1W0pButZmX1YdHT5g', #Consumer Secret (API Secret)
...
setup_twitter_oauth("swFC3PYt7hh5F6EGtPg7ATnQx", # Consumer Key (API Key)
                    "682qgTTOArAShjpTqfkjFvH27Zj4NEvhz1W0pButZmX1YdHT5g", #Consumer Secret (API Secret)
                    "570989918-VfjD9Tr4BDOLIMBlZZUQ9bzrQiE75JQbCrSPu5LK", "JeiuyQFkhltsA3npJYVMiMaevjY49wQtMxJprtf70E9HK")  # Access Token)  #Access Token Secret
...
pos.words=scan("E:\\Assignment\\11) Text mining\\positive-words.txt", what="character", comment.char=";")	# read-in positive-words.txt
neg.words=scan("E:\\Assignment\\11) Text mining\\negative-words.txt", what="character", comment.char=";") 	# read-in negative-words.txt
pos.words=c(pos.words,"wow", "kudos", "hurray") 			# including our own positive words to the existing list
neg.words = c(neg.words)
stopwdrds = readLines("E:\\Assignment\\11) Text mining\\stop.txt")


# Loading the text file to be analysed 
x = readLines("E:\\Assignment\\11) Text mining\\stop.txt") 	
